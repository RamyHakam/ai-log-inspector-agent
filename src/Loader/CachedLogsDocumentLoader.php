...
class CachedLogsDocumentLoader implements LoaderInterface
{
    ...
    private function loadSingleFile(string $filename): iterable
    {
        ...
        if (!is_file($filePath)) {
            throw new InvalidArgumentException(sprintf('Log file "%s" does not exist in cache directory.', $filename));
        }
    ...
    private function loadAllFiles(array $options): iterable
    {
        ...
        if (false === $files) {
            throw new RuntimeException(sprintf('Failed to scan cache directory "%s" for log files.', $this->cacheDir));
        }

        if (empty($files)) {
            throw new RuntimeException(sprintf('No log files found in cache directory "%s" matching pattern "%s".', $this->cacheDir, $pattern));
        }
